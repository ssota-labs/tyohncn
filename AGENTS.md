...
- **CLI contract:** external mode only — never `transformStyleMap` / strip `cn-*`. `apply --style` swaps CSS only (builtin or pack source). `pack add/new/validate` for community design packs. `studio` opens project-bound Studio (`TYOHN_PROJECT_ROOT`).
